...
class DocumentManager
{
    ...
    public function setDocument($id_doc): void
    {
        try {
            $sqlParams = "	SELECT 
								document.*, 
    ...
    protected function clearAttributes($clearRule = true): void
    {
    ...
    public function setTypeError($typeError): void
    {
        $this->typeError = $typeError;
    }

    public function setRuleId($ruleId): void
    {
        $this->ruleId = $ruleId;
    }

    public function setDocIdRefError($docIdRefError): void
    {
    ...
    protected function checkNoChange($history): bool
    {
        try {
            ...
            if (!empty($this->ruleFields)) {
                foreach ($this->ruleFields as $field) {
                    if (trim($history[$field['target_field_name']]) != trim($target[$field['target_field_name']])
                        ...
                        if (empty($history[$field['target_field_name']])
            ...
            if (!empty($this->ruleRelationships)) {
                foreach ($this->ruleRelationships as $ruleRelationship) {
                    if ('Myddleware_element_id' != $ruleRelationship['field_name_target']   // No check change on field Myddleware_element_id
            ...
            if (!empty($fields)) {
                foreach ($fields as $ruleField) {
                    if ('S' == $type) {
                        ...
                        if (!array_key_exists($ruleField['target_field_name'], $data)
                            and 'H' == $type
                        ) {
                            continue;
                        }
    ...
    public function getTransformValue($source, $ruleField)
    {
        try {
            ...
            elseif ('Myddleware_element_id' == $ruleField['source_field_name']
                and isset($source['id'])
            ) {
    ...
    public function setRuleParam(): void
    {
        try {
            $sqlParams = 'SELECT * 
							FROM ruleparam 
        ...
        try {
            ...
            if (!empty($this->ruleRelationships)) {
                ...
                foreach ($this->ruleRelationships as $ruleRelationship) {
                    ...
                    if ('Myddleware_element_id' == $ruleRelationship['field_name_target']
                        ...
                        if (!empty($this->data[$ruleRelationship['field_name_source']])
                            || (
     ...
     * @throws \Doctrine\DBAL\Exception
     */
    public function updateStatus($new_status): void
    {
     ...
     * @throws \Doctrine\DBAL\Exception
     */
    public function updateDeleteFlag($deleted): void
    {
        $this->connection->beginTransaction(); // -- BEGIN TRANSACTION
        try {
            $now = gmdate('Y-m-d H:i:s');
    ...
    protected function afterStatusChange($new_status): void
    {
    }
     ...
     * @throws \Doctrine\DBAL\Exception
     */
    public function updateType($new_type): void
    {
        $this->connection->beginTransaction(); // -- BEGIN TRANSACTION
        try {
            $now = gmdate('Y-m-d H:i:s');
    ...
    protected function searchRelateDocumentByStatus($ruleRelationship, $record_id, $status): array|bool|null
    {
        try {
            $direction = $this->getRelationshipDirection($ruleRelationship);
            // En fonction du sens de la relation, la recherche du parent id peut-être inversée (recherchée en source ou en cible)
     ...
     * @throws \Doctrine\DBAL\Exception
     */
    protected function createDocLog(): void
    {
        $this->connection->beginTransaction(); // -- BEGIN TRANSACTION
        try {
            $now = gmdate('Y-m-d H:i:s');
