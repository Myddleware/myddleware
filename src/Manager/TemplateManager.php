...
class TemplateManager
{
    ...
    public function convertTemplate(
        ...
        try {
            ...
            $this->requestStack->getCurrentRequest()->getSession()->set('info', [$this->translator->trans('messages.template.nb_rule').$nbRule, $this->translator->trans('messages.template.help')]);
        } catch (Exception $e) {
            // Rollback in case of error
            $this->entityManager->getConnection()->rollBack(); // -- ROLLBACK TRANSACTION
            $this->requestStack->getCurrentRequest()->getSession()->set('error', [$this->translator->trans('error.template.creation'), $e->getMessage()]);
