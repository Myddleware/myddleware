...
class sage50core extends solution
{
    ...
    protected $xml;
    protected $dataToHTML = [];
    protected $required_fields = ['default' => ['id', 'updated', 'published']];
    protected $update;
    protected $subModules = ['salesOrderLine', 'salesInvoiceLine', 'purchaseOrderLine', 'purchaseOrderDeliveryLine'/* ,'bankAccount' */];
    protected $parentModules = ['salesOrder', 'salesInvoice'/* ,'tradingAccount' */];
    protected $createOnlyModules = ['salesOrder', 'salesInvoice'];
    protected $readLimit = 100;
    protected $linkFound; // temp variable
    protected $linkData; // temp variable

    protected $moduleSubFields = [
        'tradingAccount' => [ // Name of the main module into Sage (first entry in the payload)
            'phoneNumber' => [ // Name of the module into Sage (first entry in the payload)
                'title' => 'phones',	// Name of the structure in the title in the link structure
                'structureNames' => ['Business Phone', 'Other Phone', 'Business Fax'], // Every type of data linked to the searchField
                'searchField' => 'type', // Search field where the structureNames are strored
                'structureFields' => ['uuid', 'text'], // Fields belonging to the structure available in Myddleware
                'additional_filter' => ['reference2' => '1'],
            ],
            'postalAddress' => [
                'title' => 'postalAddresses',
                'structureNames' => ['Billing', 'Shipping', 'Registered'],
                'searchField' => 'type',
                'structureFields' => ['uuid', 'active', 'name', 'address1', 'address2', 'address3', 'address4', 'townCity', 'stateRegion', 'stateRegion', 'zipPostCode', 'country', 'primacyIndicator', 'primacyIndicator', 'description'],
            ],
            'email' => [
                'title' => 'emails',
                'structureNames' => ['Supplier Registered Email', 'Supplier Registered Email2', 'Supplier Registered Email3', 'Supplier Delivery Email', 'Customer Registered Email', 'Customer Delivery Email', 'Customer Registered Email2', 'Customer Registered Email3'],
                'searchField' => 'label',
                'structureFields' => ['uuid', 'address'],
            ],
            'contact' => [
                'title' => 'contacts',
                'structureNames' => ['Customer Delivery Contact', 'Supplier Delivery Contact'],
                'searchField' => 'type',
                'structureFields' => ['uuid', 'fullName', 'salutation', 'firstName', 'familyName'],
            ],
            'bankAccount' => [
                'title' => 'bankAccounts',
                'structureFields' => ['uuid', 'active', 'name', 'description', 'branchIdentifier', 'accountNumber', 'iBANNumber', 'bICSwiftCode', 'rollNumber', 'currency', 'operatingCompanyCurrency', 'paymentAllowedFlag', 'receiptAllowedFlag'],
            ],
            'financialAccount' => [
                'title' => 'financialAccounts',
                'structureFields' => ['uuid', 'active', 'reference', 'reference2', 'name', 'account', 'description', 'currency', 'balance'],
            ],
            'taxCode' => [
                'title' => 'taxCode',
                'structureFields' => ['uuid', 'reference', 'type', 'value', 'valueText'],
            ],
        ],
        'bankAccount' => [ // Name of the main module into Sage
            'tradingAccount' => [
                'title' => 'tradingAccount',
                'structureFields' => ['uuid', 'active', 'customerSupplierFlag', 'companyPersonFlag', 'reference', 'name'],
            ],
            'postalAddress' => [
                'title' => 'postalAddress',
                'structureNames' => ['Other'],
                'searchField' => 'type',
                'structureFields' => ['uuid', 'active', 'name', 'address1', 'address2', 'address3', 'address4', 'townCity', 'stateRegion', 'stateRegion', 'zipPostCode', 'country', 'primacyIndicator', 'primacyIndicator', 'description'],
            ],
        ],
        'salesOrder' => [ // Name of the main module into Sage
            'tradingAccount' => [
                'title' => 'tradingAccount',
                'structureFields' => ['uuid', 'active', 'customerSupplierFlag', 'companyPersonFlag', 'reference', 'name'],
            ],
            'postalAddress' => [
                'title' => 'postalAddresses',
                'structureNames' => ['Billing', 'Shipping'],
                'searchField' => 'type',
                'structureFields' => ['uuid', 'active', 'name', 'address1', 'address2', 'address3', 'address4', 'townCity', 'stateRegion', 'stateRegion', 'zipPostCode', 'country', 'primacyIndicator', 'primacyIndicator', 'description'],
            ],
            'contact' => [
                'title' => 'buyerContact',
                'structureFields' => ['uuid', 'reference', 'fullName', 'firstName', 'familyName'],
            ],
        ],
    ];
    ...
    public function login($paramConnexion)
    {
        ...
        try {
            ...
            if (0 === $response['curlErrorNumber']) { // url all fine . precced as usual
                if (!empty($response['curlInfo']) && 200 === $response['curlInfo']['http_code']) { // token is valid
                    $this->connexion_valide = true;
                    $this->setAccessToken($this->token);
                    $xml = simplexml_load_string($response['curlData']);
                    $this->setXML($xml);
                } elseif (!empty($response['curlInfo']) && 401 === $response['curlInfo']['http_code']) { //if 401 non unauthorized
            ...
            if ($xml) {
                ...
                if (count($fields) > 0) { // url all fine . precced as usual
                    foreach ($fields as $key => $field) {
                        ...
                        if (
                                (
                                    '--type' == substr($field['type'], -6)
                                 or '--list' == substr($field['type'], -6)
                                )
                        ) {
                                ...
                                if (!empty($fieldList['structureNames'])) {
                                    foreach ($fieldList['structureNames'] as $structureName) {
                                        foreach ($fieldList['structureFields'] as $structureField) {
                                            if ('uuid' == $structureField) {
                                                $this->fieldsRelate[$structureKey.'__'.str_replace(' ', '', $structureName).'__'.str_replace(' ', '', $structureField)] = ['label' => $structureKey.' : '.$structureName.' - '.$structureField, 'type' => 'varchar(255)', 'type_bdd' => 'varchar(255)', 'required_relationship' => 0];
                                            } else {
                                                $this->moduleFields[$structureKey.'__'.str_replace(' ', '', $structureName).'__'.str_replace(' ', '', $structureField)] = ['label' => $structureKey.' : '.$structureName.' - '.$structureField, 'type' => 'varchar(255)', 'type_bdd' => 'varchar(255)', 'required' => 0];
                                            }
                                        }
                                    }
                                } else {
                                    foreach ($fieldList['structureFields'] as $structureField) {
                                        if ('uuid' == $structureField) {
                                            $this->fieldsRelate[$structureKey.'__'.str_replace(' ', '', $structureField)] = ['label' => $structureKey.' - '.$structureField, 'type' => 'varchar(255)', 'type_bdd' => 'varchar(255)', 'required_relationship' => 0];
                                        } else {
                                            $this->moduleFields[$structureKey.'__'.str_replace(' ', '', $structureField)] = ['label' => $structureKey.' - '.$structureField, 'type' => 'varchar(255)', 'type_bdd' => 'varchar(255)', 'required' => 0];
                                        }
                                    }
                                }
                            }
                            ...
                            $this->moduleFields[(string) $field['name']] = ['label' => (string) $field['name'], 'type' => $typeData, 'type_bdd' => 'varchar(255)', 'required' => 0];
                        }
                    }
                }
                ...
                if ('target' == $type) {
                    ...
                    if (count($relateFields) > 0) { // url all fine . precced as usual
                        foreach ($relateFields as $key => $field) {
                            $this->fieldsRelate[(string) $field['name']] = ['label' => (string) $field['name'], 'type' => 'varchar(255)', 'type_bdd' => 'varchar(255)', 'required_relationship' => 0];
                        }
                    }
                }
    ...
    public function read($param)
    {
        try {
            ...
            if (!empty($response['curlInfo']) && 200 === $response['curlInfo']['http_code']) { // token is valid
                ...
                if (!empty($response['curlData']['entry'])) {
                    ...
                    foreach ($response['curlData']['entry'] as $record) {
                        ...
                        foreach ($param['fields'] as $field) {
                            if (in_array($field, ['updated', 'published'])) {
                                continue;
                            }
                            // If the field is an array, it means that there is a structure, and we have to call Sage to get the detail of this structure (e.g. phoneNumber__BusinessPhone__text)
                            $fieldArray = explode('__', $field);
                ...
                if (false === strpos($response['curlData']['message'], "Unable to locate the specified object ''")) {
            ...
            if ($result['count'] == $param['limit']) {
                $result['ruleParams'][] = ['name' => 'Offset', 'value' => $startIndex + $result['count']];
                $result['ruleParams'][] = ['name' => 'OffsetDateREf', 'value' => $param['date_ref']];
            } else {
                $result['ruleParams'][] = ['name' => 'Offset', 'value' => 0];
                $result['ruleParams'][] = ['name' => 'OffsetDateREf', 'value' => ''];
            }
        } catch (\Exception $e) {
...
<entry xmlns:sdata="http://schemas.sage.com/sdata/2008/1" 
  ...
  <sdata:payload>
                    ...
                    // If the field is a submodule
                        ...
                        if (in_array($key, $this->subModules)) {
                            ...
                            foreach ($value as $subIdDoc => $subData) {
                                ...
                                $subDocIdArray[$subIdDoc] = ['id' => $uuid];

                                $xmlData .= '<'.$key.' sdata:uuid="'.$uuid.'">'.chr(10);
                                foreach ($subData as $subKey => $subValue) {
                                    if (in_array($subKey, [$param['module'], 'id_doc_myddleware', 'target_id', 'source_date_modified'])) {
                                        continue;
                                    }
                                    // If relate field or commodity (submodule)
                                    if (!empty($this->fieldsRelate[$subKey]) or 'commodity' == $subKey) {
     ...
     * @param string     $method
     * @param null       $data
     * @param null|mixed $xml
     * @param mixed      $read_last
     *
     * @return array
     *
     * @throws \Exception
     */
    public function makeRequest($server, $token, $path, $method, $args = null, $xml = null, $read_last = false)
    {
        if (function_exists('curl_init') && function_exists('curl_setopt')) {
            ...
            } elseif ('create' == $method or 'update' == $method) {
                $result = str_replace('sdata:', '', $result);
                $result = str_replace('crm:', '', $result);
                $result = str_replace('sc:', '', $result);
