...
class bittlecore extends solution
{
    ...
    public function login($paramConnexion)
    {
        ...
        try {
            ...
            if (!isset($restponse['result'])) {
                throw new \Exception('Failed to connect bittle, call error.');
            } // Si on a pas d'index "result" dans la réponse de Bittle on renvoie une erreur
            // Faire un apple vers Bittle, par exemple le récupération de tous les connectors : URL: company/ {idCompany}/user/connector/
            if ('Success' == $restponse['result']) { // Si Bittle nous renvoie Success alors connexion valide
    ...
    public function get_modules($type = 'source')
    {
        try {
            ...
            $modules = ['Container' => 'Bittle container (new)'];
    ...
    public function get_module_fields($module, $type = 'source')
    {
        ...
        try {
            ...
            $this->moduleFields['Date'] = ['label' => 'Date', 'type' => 'varchar(255)', 'type_bdd' => 'varchar(255)', 'required' => ('Container' == $module ? 1 : 0)];
            ...
            if ('Container' == $module) {
                ...
                $this->moduleFields['Metric'] = ['label' => 'Metric', 'type' => 'varchar(255)', 'type_bdd' => 'varchar(255)', 'required' => 1];
            }
    ...
    public function get_module_fields_relate($module, $param)
    {
        ...
        if (!empty($ruleFields)) {
            foreach ($ruleFields as $ruleField) {
                $this->fieldsRelate[$ruleField['target_field_name']] = [
            ...
            foreach ($param['data'] as $idDoc => $data) {
                try {
                    ...
                    foreach ($data as $key => $value) {
                        ...
                        $rows .= ' c'.$c.'="'.('DATE' == $mappingType ? $this->dateTimeFromMyddleware($value) : $value).'"'; // Ajout de la valeur dans la définition de la balise <Row/>
                    ...
                    if (!empty($connector['data']['ConnectorModel']['mappings']['mapping'])) {
                        foreach ($connector['data']['ConnectorModel']['mappings']['mapping'] as $filed) {
                            ...
                            if (substr($filed['fileField'], 0, strlen($param['rule']['module_source'])) != $param['rule']['module_source']) {
                                ...
                                $rows .= ' c'.$c.'="'.('METRIC' == $mappingType ? 0 : '').'"'; // Ajout de la valeur dans la définition de la balise <Row/>
                                ++$c; // Incrémentation du compteur
                            }
                        }
                    }
    ...
    protected function checkConnector($param)
    {
        ...
        if (
                '001' != $param['rule']['version']
            || (
                    !empty($param['content']['params']['group'])
                && 'child' == $param['content']['params']['group']
            )
        ) {
            ...
            if (empty($connectorId)) {
                $this->messages[] = ['type' => 'error', 'message' => 'Failed to find a connector in Bittle for this Rule. The connector is not updated in Bittle.'];
            }
            /*
            ...
            foreach ($mapping as $bittleField) {
                if ('Row ID' == $bittleField['displayName']) {
                    continue;
                }
            ...
            foreach ($param['ruleFields'] as $ruleField) {
                ...
                if (!in_array($param['rule']['module_source'].'_'.$ruleField['target_field_name'], $bittleFields)) {
                    $diff[] = ['displayName' => $fieldName, 'TYPE' => $mappingType, 'fileField' => $param['rule']['module_source'].'_'.$ruleField['target_field_name']];
                }
            }
            if (empty($diff)) {
                $this->messages[] = ['type' => 'success', 'message' => 'No added field on your rule. The connector has not been changed in Bittle. '];
            ...
            foreach ($diff as $fieldDiff) {
                ...
                $xml .= '<mapping>
								...
								'.('DATE' == $fieldDiff['TYPE'] ? '<pattern>dd/MM/yyyy hh:mm</pattern>' : '').'
            ...
            if ('Success' == $response['result']) {
                $this->messages[] = ['type' => 'success', 'message' => 'Connector '.$ConnectorName.' successfully updated in Bittle. Fields added : '.$fieldstext.' .'];
    ...
    protected function createBittleConnector($param)
    {
        ...
        foreach ($param['ruleFields'] as $ruleField) {
            ...
            $xml .= '<mapping>
							...
							<displayName>'.(in_array($ruleField['target_field_name'], ['Metric', 'Date']) ? $ruleField['source_field_name'] : $fieldName).'</displayName>
							<mappingType>'.$mappingType.'</mappingType>
							'.('DATE' == $mappingType ? '<pattern>dd/MM/yyyy hh:mm</pattern>' : '').'
						</mapping>';
        }
        $xml .= '</mappings>
			   </ConnectorModel>';
        ...
        if ('Success' == $response['result']) {
            $this->messages[] = ['type' => 'success', 'message' => 'Connector '.$param['rule']['name'].' successfully created in Bittle. '];
            if (!empty($response['data']['ConnectorModel']['idConnector'])) {
    ...
    protected function call($method, $parameters = [])
    {
        ...
        if (empty($result)) {
            throw new \Exception('Call returned an empty response.');
        } // Traitement d'erreur si on a une réponse vide
        ob_end_flush();
    ...
    {
        if (false !== stripos($field, 'Date')) {
            return 'DATE';
        }
        if (false !== stripos($field, 'Filter')) {
            return 'FILTER';
        }
        // Les champs référence sont considéré comme des filtres et permettent de lier plusieurs règles
        if (false !== stripos($field, 'Reference')) {
            return 'FILTER';
        }
        if (false !== stripos($field, 'Metric')) {
            return 'METRIC';
        }
    }
    ...
    public function beforeRuleSave($data, $type)
    {
            ...
            if (!empty($referenceFields)) {
                foreach ($referenceFields as $referenceField) {
                    ...
                    if (empty($data['content']['fields']['name'][$referenceField['field_name_target']])) {
                        return ['done' => false, 'message' => 'The field '.$referenceField['field_name_target'].' is linked to the rule '.$referenceField['name'].'. Change this rule before removing this field.'];
                    }
                }
            }
        }
        ...
        if (
                'Container' != $data['module']['target']['name']
            && empty($data['relationships'])
        ) {
            return ['done' => false, 'message' => 'Failed to save the rule. You have to create a relationship with the Connector '.$data['module']['target']['name'].' that you selected in the first step.'];
        }
        ...
        if (!empty($data['relationships'])) {
            ...
            if (empty($fetch['rule_id'])) {
                return ['done' => false, 'message' => 'Failed to save the relationship. The field '.$data['relationships'][0]['target'].' doesn\'t belong to the selected rule ('.$data['relationships'][0]['rule'].'). Change the relationShip to save this rule. '];
            }
            // Ajout du paramètre child à la règle puisqu'une relation existe
            return ['done' => true, 'message' => '', 'params' => ['group' => 'child']];
        }

        // Ajout du paramètre root à la règle puisqu'aucune relation n'existe
        return ['done' => true, 'message' => '', 'params' => ['group' => 'root']];
    ...
    public function afterRuleSave($data, $type)
    {
        try {
            ...
            if (false === $this->connexion_valide) {
                $this->messages[] = ['type' => 'error', 'message' => 'Failed to login to Bittle. The connector is not created in Bittle. '];
            }
            ...
            if (empty($data['rule'])) {
                $this->messages[] = ['type' => 'error', 'message' => 'Failed to retrieve the rule in the database. The connector is not created in Bittle. '];
            }
            ...
            if (empty($data['ruleFields'])) {
                $this->messages[] = ['type' => 'error', 'message' => 'Failed to retrieve the ruleFields in the database. The connector is not created in Bittle. '];
            }
            ...
            $error = $e->getMessage();
            $this->messages[] = ['type' => 'error', 'message' => 'Failed to create the connector in Bittle : '.$e->getMessage()];
        }
