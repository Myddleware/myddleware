...
class medialogistiquecore extends solution
{
    ...
    protected $allowParentRelationship = ['gestion_commande'];
    ...
    public function read($param)
    {
        try {
            ...
            $timestamp = gmdate('U');
            // Build login parameters
            $moduleCall = ('gestion_article' == $param['module'] ? $param['module'] : 'gestion_commande');
            $parameters = [
                'id_client' => $this->paramConnexion['clientid'],
                'id_auth' => $this->paramConnexion['authid'],
                'expires' => $timestamp,
            ...
            while (!$lastCall) {
                ...
                if (
                     ...
                     or 'gestion_article' == $moduleCall // We don't get historic for stock
                ) {
                ...
                if (!empty($resultQuery->ok)) {
                    foreach ($resultQuery->data as $recordData) {
                        ...
                        if ('suivi_commande' == $param['module']) {
        ...
        try {
            foreach ($param['data'] as $idDoc => $data) {
                try {
                    ...
                    if ('gestion_commande' == $param['module']) {
    ...
    protected function createOrder($param, $idDoc)
    {
        ...
        if (!empty($order['gestion_commande'])) {
            foreach ($order['gestion_commande'] as $subOrderObjectId => $subOrderObjectValue) {
                ...
                $subDocIdArray[$subOrderObjectId] = ['id' => uniqid('', true)];
            ...
            $resultCall = $this->call($this->url.'gestion_commande?'.http_build_query($parameters), 'POST', ['csv' => $csv]);

            // Error management
            if (1 == $resultCall->ok and 'OK' == $resultCall->data[0]->msg) {
                $result[$idDoc] = ['id' => $resultCall->data[0]->ref_cmd, 'error' => ''];
                ...
                $result[$idDoc] = ['id' => -1, 'error' => $resultCall->msg];
            // Order error management
            } elseif (!empty($resultCall->data[0]->msg)) {
                $result[$idDoc] = ['id' => -1, 'error' => $resultCall->data[0]->msg];
            } else {
                $result[$idDoc] = ['id' => -1, 'error' => 'No result from MVS. '];
            }
    ...
    public function getRuleMode($module, $type)
    {
        ...
        if (
                'target' == $type
            && in_array($module, ['gestion_commande'])
        ) {
            return [
                'C' => 'create_only',
            ];
