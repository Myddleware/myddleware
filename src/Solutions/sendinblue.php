...
class sendinblue extends solution
{
    ...
    public function login($paramConnexion)
    {
        ...
        try {
            $this->logDebug('sendinblue login request');
            $result = $apiInstance->getAccount();
            $this->logDebug('sendinblue login response', ['email' => $result->getEmail()]);
    ...
    public function get_module_fields($module, $type = 'source', $param = null): array
    {
        ...
        try {
            ...
            $this->logDebug('sendinblue getAttributes request');
    ...
    public function read($param)
    {
        ...
        switch ($param['module']) {
			case 'transactionalEmailActivity':
				...
				$exit = false;				
				do {
                    $this->logDebug('sendinblue getEmailEventReport request', ['offset' => $offset, 'event' => $event]);
                    ...
                    $this->logDebug('sendinblue getEmailEventReport response');
            ...
            case 'contactUnsubscriptions':
                ...
                if (
                 ...
                 or !empty($param['query']['SMS'])
                    ...
                    try {
                        ...
                        $this->logDebug('sendinblue getContactInfo request', ['key' => $shearchKey]);
                        $resultApi = $apiInstance->getContactInfo($shearchKey);
                        $this->logDebug('sendinblue getContactInfo response');
                    } catch (\Exception $e) {
                        // Search with second key if not found with first key
                        if (!empty($shearchKey2)) {
                            try {
                                $this->logDebug('sendinblue getContactInfo request', ['key' => $shearchKey2]);
				...
				// Search all contact modified after the reference date
					...
					do {
						$recordsCall = [];
						$this->logDebug('sendinblue getContacts request', ['limit' => $this->limitCallContact, 'offset' => $offset]);
						$resultApi = $apiInstance->getContacts($this->limitCallContact, $offset, $modifiedSince, 'asc');
						$this->logDebug('sendinblue getContacts response');
    ...
    {    
		try {
			...
			$this->logDebug('sendinblue createContact request', ['email' => $record['email']]);
			$result = $apiInstance->createContact($createContact);
			$this->logDebug('sendinblue createContact response', ['id' => $result->getId()]);
		} catch (\Exception $e) {
    ...
    protected function update($param, $record, $idDoc = null)
    {
        try {
            ...
            $this->logDebug('sendinblue updateContact request', ['identifier' => $identifier]);
            $result = $apiInstance->updateContact($identifier, $updateContact);
            $this->logDebug('sendinblue updateContact response');
        } catch (\Exception $e) {
    ...
    protected function delete($param, $record)
    {
        try {
            ...
            $identifier = $record['target_id'];
            $updateContact['attributes'] = $record;
            $this->logDebug('sendinblue deleteContact request', ['identifier' => $identifier]);
            $result = $apiInstance->deleteContact($identifier);
            $this->logDebug('sendinblue deleteContact response');
        } catch (\Exception $e) {
