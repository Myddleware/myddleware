...
class brevocore extends solution
{
    protected $config;
    ...
    public function login($paramConnexion)
    {
        ...
        try {
            ...
            if (!empty($result->getEmail())) {
                ...
                return ['error' => 'Failed to connect to brevo: '.$result->message];
            }
        } catch (\Exception $e) {
            $error = $e->getMessage();
    ...
    {
        $result = [];
        // Function are differents depending on the type of record we read from brevo
        switch ($param['module']) {
			case 'transactionalEmailActivity':
                ...
                // As we build the id (it doesn't exist in brevo), we add it to the param field
                $param['fields'][] = 'id';
                ...
                if (!empty($param['query']['id'])) {
                    ...
                    if (!empty($searchParam[0])) {
                        ...
                        throw new \Exception('No event found in the id  '.$param['query']['id'].'. Failed to search the record into brevo');
                    }
        ...
        //Recover all contact from brevo
        if (!empty($records)) {
...
/*     protected function getDateEnd($dateObj): string
    ...
    {    
		try {
			// Import or create new contact for brevo
			$apiInstance = new \SendinBlue\Client\Api\ContactsApi(new \GuzzleHttp\Client(), $this->config);
