...
class DocumentRepository extends ServiceEntityRepository
{
    ...
    {
        $this->debugLogger->logStart(__CLASS__, __FUNCTION__, ['filters' => $filters]);
        $__debugReturn = null;
        try {
            $qb = $this->createQueryBuilder('d');
            $qb->select('d')
            ...
            return $__debugReturn = $qb->getQuery()->setMaxResults(1)->getOneOrNullResult();
        } finally {
            $this->debugLogger->logEnd(__CLASS__, __FUNCTION__, $__debugReturn);
        }
    }
    ...
    {
        $this->debugLogger->logStart(__CLASS__, __FUNCTION__, ['action' => $action, 'dataType' => $dataType, 'ids' => $ids, 'forceAll' => $forceAll, 'fromStatus' => $fromStatus]);
        $__debugReturn = null;
        try {
            $qb = $this->createQueryBuilder('d')
                ->select('d')
    ...
    {
        $this->debugLogger->logStart(__CLASS__, __FUNCTION__, ['rule' => $rule, 'document' => $document, 'id' => $id, 'type' => $type]);
        $__debugReturn = null;
        try {
            $qb = $this->createQueryBuilder('d');
            $qb
