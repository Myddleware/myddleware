...
class databasecore extends solution { 
	...
	public function read($param) {		
		...
		try {
			...
			$result = $this->searchDeletionByCompare($param, $result);
		}
		catch (\Exception $e) {
		    $result['error'] = 'Error : '.$e->getMessage().' '.$e->getFile().' Line : ( '.$e->getLine().' )';
		}	
	...
	protected function searchDeletionByCompare($param, $result) {
		...
		try {
			...
			if(!empty($fieldsSource)) {
				if ($type == 'source'){
					...
					$deletionParam['option']['compareTable'] = 'Compare source table with Myddleware (low performance)';
