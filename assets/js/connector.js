...
$( function() {
    ...
    $('#connexion').on('click', function(){
        ...
        $('.champs .row').each(function(){
            var labelObj = $(this).find('label').first();
            var labelText = labelObj.text().toLowerCase();

            if(labelText != 'solution' && labelText != 'nom' && labelText != 'connexion') {              
                var input = $(this).find('.params[data-param]');
                
                if (input.length > 0) {
                    var dataParam = input.attr('data-param');
                    var inputValue = input.val();
                    
                    if (dataParam !== undefined && inputValue !== undefined) {
                        datas += dataParam + "::" + inputValue.replace(/;/g, "") + ";";
                    }
                }
            }           
        }); 
                ...
                $.ajax({
                    ...
                    success: function(data){
                        ...
                        if(param[0] == 1) { 
                            ...
                            if(!json.success) {                             
                                status.html('<i class="fa fa-lightbulb status-offline"></i>');
                                $('#msg_status span.error').html(json.message); // Modifié car 'r' n'existe pas ici
                                $('#msg_status').show();
                            }
                            else{
                                status.html('<i class="fa fa-lightbulb status-online"></i>');
